...
* Capacitors: $1.2nF$ (two) and $8.2nF$ or values that are close; $1\mu F$ 
...
:height: 400px
Schematic of the nMOS differential pair with a 8x $I_{REF}$ current bias. Note the location of the load capacitors $C_L$ and the common-source capacitor $C_{CS}$. The capacitors will not always be used for all parts of the experiments. 
```
- Build the circuit:
...
We will start with measurements on the oscilloscope at low frequencies to determine an appropriate input amplitude for the network analyzer measurements. Preferably we also want to null the offset at the output. The oscilloscope measurements can be used to do a sanity check on our network analyzer measurements later. 

- Nulling the offset:
    - Connect the signal generator *W1* to *INp* and $V_{REF}$ to *INn*.
    - Generate a constant 1.25V voltage with the signal generator *W1*.
    - Observe $V_{OUTn}$ and $V_{OUTp}$; there will likely be an offset; adjust $V_{REF}$ to minimize the output offset. Measure and note the new $V_{REF}$ and make sure its value is reasonable. 
- Differential Gain:
    - Generate a 300Hz 200mVpp sinewave with a 1.25V DC offset with *W1*. **Disconnect the signal generator first and verify the signal on the oscilloscope before applying it to your circuit. Do not apply signals above $2.5V$ or below $0V$ to the chip** 
    - Connect the oscilloscope channels *1+* and *2+* to the outputs and ground *1-* and *2-*.
    - Create two mathematical channels[^math] in the oscilloscope, namely the differential mode (channel2 - channel 1) and the common mode (0.5*(channel1 + channel2)) and turn on the measurements.
...
- Differential Gain vs Frequency:
    ...
    - Set up the Network Analyzer to use channel 1 as the reference and channel 2 as the output. Make sure to specify a signal with a 1.25V offset and 100mV amplitude[^na_amp]. Turn on DC filtering at set the settling times to $20ms$. Increase the number of periods and averages if needed. 
...
- Common-Mode Gain vs Frequency:
    ...
    - Measure the $A_{cc}$ between $100Hz$ and $10MHz$. 
        - Measure and compare the gain and bandwidth for $C_L$ is $0$ and $1.2nF$ and while $C_{CS}$ is $0$. How do the results compare to your estimates and to the oscilloscope measurements.
        - Now add a $C_{CS}$ of $8.2nF$ and repeat the measurements. Can you explain the differences?

- Response at the Common Source:
    - Keep the circuit in the common-mode configuration. 
    - Measure the $V_{CMSRC}$ vs $V_{IN}$ with and without $C_{CS}$. Can you explain the responses?
...
```
- Differential Load:
    - Measure $A_{dd}$ and $A_{cc}$ with load capacitors connected as shown in the figure above and with $C_{CS}$ present. Discuss the CMRR vs frequency. 
